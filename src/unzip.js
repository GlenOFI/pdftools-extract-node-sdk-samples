// https://raw.githubusercontent.com/thejoshwolfe/yauzl/master/examples/unzip.js
...
//   }
    ...
    if(/\//.test(zipFilePath)){
      zipFileName = /.*\/(.*)\..*/.exec(zipFilePath)
  }
  else{
    zipFileName = /.*\b(.*)\..*/.exec(zipFilePath)
  }
    outFilePath = `${arg}/${zipFileName[1]}`;
  }
}
if (zipFilePath == null || /^-/.test(zipFilePath) || (lenArg != null && endArg != null)) {
  console.log(
    "usage: node unzip.js [options] path/to/file.zip path/to/extract/to\n" +
    "\n" +
    "unzips the specified zip file into the current directory (default) or the specified path using the file name\n" +
    "\n" +
    "options:\n" +
    "  --offset START\n" +
    "  --len LEN\n" +
...
console.log(`Extract to: ${outFilePath}`)
...
function handleZipFile(err, zipfile) {
  ...
  zipfile.on("entry", function(entry) {
    var fileName = `${outFilePath}/${entry.fileName}`;
